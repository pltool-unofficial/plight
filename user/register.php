...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 验证：用户名 3-50、临时账户名 1-50、密码至少 8 位且需含字母和数字
    if ($username === '' || strlen($username) < 3 || strlen($username) > 50) {
        ...
        $error = '密码需至少包含字母和数字';
    } elseif ($tempUsername === '' || strlen($tempUsername) > 50) {
        $error = '临时账户名长度需为1-50字符';
    } else {
    ...
    <div class="auth-box">
        ...
        <form method="POST" class="form">
            ...
            <div class="form-group">
                ...
                <input type="text" id="username" name="username" required minlength="3" maxlength="50" value="<?= escapeHtml($_POST['username'] ?? '') ?>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="password">密码 (至少8位，需含字母和数字)</label>
                <input type="password" id="password" name="password" required minlength="8">
            </div>
            <div class="form-group">
                <label for="temp_username">临时账户名</label>
                <input type="text" id="temp_username" name="temp_username" maxlength="50" placeholder="请填写您的物实临时账户名" value="<?= escapeHtml($_POST['temp_username'] ?? '') ?>">
